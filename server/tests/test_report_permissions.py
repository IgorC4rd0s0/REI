...
class ReportPermissionTests(unittest.TestCase):
    ...
    def assert_rejected(self, status: int, code: str, callback) -> rei_server.ReportWriteRejected:
    ...
    def test_implantador_cannot_change_another_implantador_report(self) -> None:
        ...
        self.assert_rejected(403, "not_report_owner", lambda: rei_server.save_report(changed, self.bob))

    def test_implantador_cannot_change_supervision_fields(self) -> None:
        original = self.payload("supervision-1", self.alice, delivery_status="Concluído")
        rei_server.save_report(original, self.alice)
        changed = copy.deepcopy(original)
        changed["report"]["fields"]["_supervisionScore"] = "9.0"
        changed["report"]["fields"]["_supervisionReviewedAt"] = "1001"
        self.assert_rejected(403, "supervision_only", lambda: rei_server.save_report(changed, self.alice))
    ...
    def test_completed_survey_is_read_only_and_cannot_regress(self) -> None:
        ...
        self.assert_rejected(409, "survey_completed", lambda: rei_server.save_report(edited, self.alice))

        regressed = copy.deepcopy(completed)
        regressed["report"]["fields"]["_stage"] = "levantamento_pendente"
        self.assert_rejected(409, "state_conflict", lambda: rei_server.save_report(regressed, self.alice))

    def test_completed_survey_answers_cannot_change_when_rei_starts(self) -> None:
        pending = self.payload("survey-transition-1", self.alice, stage="levantamento_pendente")
        ...
        self.assert_rejected(409, "survey_completed", lambda: rei_server.save_report(implementation, self.alice))
    ...
    def test_only_supervisor_can_evaluate_and_evaluation_is_locked(self) -> None:
        ...
        self.assert_rejected(403, "supervision_only", lambda: rei_server.save_report(evaluation, self.alice))
        ...
        self.assert_rejected(409, "evaluation_locked", lambda: rei_server.save_report(replacement, self.supervisor))
    ...
    def test_supervisor_cannot_evaluate_unfinished_report(self) -> None:
        ...
        self.assert_rejected(422, "invalid_evaluation", lambda: rei_server.save_report(evaluation, self.supervisor))
    ...
    def test_forward_skip_is_422_and_completed_report_cannot_be_reopened(self) -> None:
        ...
        self.assert_rejected(422, "invalid_transition", lambda: rei_server.save_report(skipped, self.alice))

        concluded = self.payload("transition-2", self.alice, delivery_status="Concluído")
        rei_server.save_report(concluded, self.alice)
        reopened = copy.deepcopy(concluded)
        reopened["report"]["deliveryStatus"] = "Não concluído"
        self.assert_rejected(409, "report_already_completed", lambda: rei_server.save_report(reopened, self.alice))

    def test_legitimate_survey_flow_and_supervisor_assignment_continue_working(self) -> None:
        survey = self.payload(
            "legitimate-1",
            self.supervisor,
    ...
    def test_supervisor_cannot_change_survey_answers(self) -> None:
        ...
        self.assert_rejected(403, "permission_denied", lambda: rei_server.save_report(changed, self.supervisor))
    ...
    def test_supervisor_must_assign_active_implantador(self) -> None:
        ...
        self.assert_rejected(422, "invalid_assignment", lambda: rei_server.save_report(survey, self.supervisor))
    ...
    def test_invalid_content_returns_422(self) -> None:
        ...
        self.assert_rejected(422, "invalid_content", lambda: rei_server.save_report(invalid, self.alice))
    ...
    def test_implantador_limit_is_applied_after_sql_ownership_filter(self) -> None:
        ...
        alice_summary = rei_server.list_reports_for_user(self.alice, limit=1, full=False)
        supervisor_latest = rei_server.list_reports_for_user(self.supervisor, limit=1, full=True)
    ...
    def test_initialize_database_backfills_legacy_payload_and_tolerates_invalid_json(self) -> None:
        legacy_payload = self.payload("legacy-1", self.alice, assigned=self.alice["username"])
        with rei_server.connect() as db:
            db.execute(
                """
                ...
                VALUES(?,?,?,?,?,?)
                """,
                ("legacy-1", "Cliente legado", 1, "2026-01-01", "2026-01-01", json.dumps(legacy_payload)),
            )
            db.execute(
                """
                INSERT INTO reports(id, client, completed_at, received_at, updated_at, payload_json)
        ...
        self.assertEqual(self.alice["username"], row["assigned_username"])
        self.assertTrue({
            "idx_reports_stage", "idx_reports_owner_username", "idx_reports_assigned_username"
        }.issubset(indexes))
